...
import HarmonyApp, { SW_ERR } from "./harmony-ledger-new"
  ...
  actions: {
    async connectLedgerApp() {
      ...
      if (response.return_code === SW_ERR) {
        throw new Error(`Authorization request rejected. One address: ${response.one_address} Return code: ${response.return_code}`)
      }
